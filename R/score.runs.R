score.runs <- function(replicates, normalized, cnas, sample.annot, genes, collapsed, normals){
	...
	# determine if the normalized data is counts or continuous (*** does this ever happen anymore? ***)	actually can use > or < 0
	discrete.data <- F;
	...
	if(discrete.data){
		scores$ari.chip <- get.ari(log10(normalized[, -c(1:3)]+1), sample.annot$cartridge);
		...
		scores$ari.pts.normcor <- get.ari(log10(replicates$norm.counts+1), replicates$pheno$Patient[replicates$pheno$type == 'Tumour']);
	}else{
		scores$ari.chip <- get.ari(normalized[, -c(1:3)]+1, sample.annot$cartridge);
		replicates$norm.counts <- replicates$norm.counts[ , replicates$pheno$SampleID[which(replicates$pheno$type == 'Tumour')]];
		colnames(replicates$norm.counts) <- substr(colnames(replicates$norm.counts),1,8);
		scores$ari.pts.normcor <- get.ari(replicates$norm.counts, replicates$pheno$Patient[replicates$pheno$type == 'Tumour']);
		}

	# type, use all patients
	if(any(sample.annot$type == 'Reference')){
		if(discrete.data){
			scores$ari.type <- get.ari(log10(normalized[ , -c(1:3)]+1), sample.annot$type);
		}else{
			scores$ari.type <- get.ari(normalized[ , -c(1:3)]+1, sample.annot$type);
			}
		}

	# replicates using CNA data
	scores$ari.pts <- get.ari(t(replicates$cnas), substr(colnames(replicates$cnas),1,8), discrete.data = TRUE);
